<?php
	...
	while($d = mysql_fetch_object($r)){
        ...
        $TickDetalhe[] = "
				<div style='float:left; width:33%;'><b style='color:#a1a1a1; font-size:14px;''>Cadastrado ID:</b> <div>".str_pad($d->codigo, 8, "0", STR_PAD_LEFT)."</div></div>".
				"<div style='float:left; width:33%;'>".((dataBr($d->data_abertura))?"<b style='color:#a1a1a1; font-size:14px;'>Data:</b><div>".dataBr($d->data_abertura)."</div>":false)."</div>".
				"<div style='float:left; width:33%;'>".(($d->status)?"<b style='color:#a1a1a1; font-size:14px;'>Situação:</b>
					<div style='color:{$cor[$d->status]}; font-weight:bold;'>".$titulo[$d->status]."</div>":false)."</div>".

				"<div style='float:left; width:33%;'><b style='color:#a1a1a1; font-size:14px;''>Peça:</b> <div>".utf8_encode($d->peca_nome)."</div></div>".
				"<div style='float:left; width:33%;'><b style='color:#a1a1a1; font-size:14px;'>Modelo:</b><div>".utf8_encode($d->modelo_nome)."</div></div>".
				"<div style='float:left; width:33%;'><b style='color:#a1a1a1; font-size:14px;'>Código:</b><div>".utf8_encode($d->codigos_nome)."</div></div>".


				"<div style='float:left; width:50%;'> <b style='color:#a1a1a1; font-size:14px;'>Setor:</b><div>".utf8_encode($d->setor_nome)."</div></div>".
				"<div style='float:left; width:50%;'> <b style='color:#a1a1a1; font-size:14px;'>Máquina:<span style='color:".(($d->parada == 's')?'red':'#333').";'> (".$parada[$d->parada].")</span></b><div>".utf8_encode($d->maquina_nome)."</div></div>".


				"<div style='float:left; width:50%;'>".(($d->time_nome)?"<b style='color:#a1a1a1; font-size:14px;'>Time:</b><div>".utf8_encode($d->time_nome)."</div>":false)."</div>".
				"<div style='float:left; width:50%;'>".(($d->motivo_nome)?"<b style='color:#a1a1a1; font-size:14px;'>Ocorrência:</b><div>".utf8_encode($d->motivo_nome)."</div>":false)."</div>".


            //    "<div style='width:100%;'> <b style='color:#a1a1a1; font-size:14px;'>Tipo de Manutenção:</b><div>".utf8_encode($d->tipo_manutencao_nome)."</div></div>".
               "<div style='width:100%;'>".(($d->problema)?"<b style='color:#a1a1a1; font-size:14px;'>Problema:</b><div>".str_replace("\n"," ",utf8_encode($d->problema))."</div>":false)."</div>".

			   "<div style='float:left; width:50%;'>".(($d->funcionario)?"<b style='color:#a1a1a1; font-size:14px;'>Funcionário:</b><div>".utf8_encode($d->funcionario)."</div>":false)."</div>".
               "<div style='float:left; width:50%;'>".(($d->tecnico)?"<b style='color:#a1a1a1; font-size:14px;'>Técnico:</b><div>".utf8_encode($d->tecnico)."</div>":false)."</div>".

               "<div style='width:100%;'>".(($d->observacao)?"<b style='color:#a1a1a1; font-size:14px;'>Observações:</b><div>".str_replace("\n"," ",$_POST['observacao'])."</div>":false)."</div><br>";

		$TickResumo[] = "<div style='float:left; width:40%;'><b style='font-size:14px;''>Cadastrado ID:</b> <div>".str_pad($d->codigo, 8, "0", STR_PAD_LEFT)."</div></div>".
						"<div style='float:left; width:60%;'>".(($d->status)?"<b style='font-size:14px;'>Situação:</b><div>".$titulo[$d->status]."</div>":false)."</div>".
						"<div > <b style='font-size:14px;'>Setor:</b><div>".utf8_encode($d->setor_nome)."</div></div>".
						"<div> <b style='font-size:14px;'>Máquina: (".$parada[$d->parada].")</b><div>".utf8_encode($d->maquina_nome)."</div></div>".
						"<div > <b style='font-size:14px;'>Time:</b><div>".utf8_encode($d->time_nome)."</div></div>".
						"<div> <b style='font-size:14px;'>Ocorrência:</b><div>".utf8_encode($d->motivo_nome)."</div></div>";
