	...
	while($d = mysql_fetch_object($r)){
		...
		if($d->status != 'c'){
			...
			$TickDetalhe[] = "
				...
				"<div style='width:100%;'>".(($d->problema)?"<b style='color:#a1a1a1; font-size:20px;'>Problema:</b><div class='detalhesTexto'>".str_replace("\n"," ",utf8_encode($d->problema))."</div>":false)."</div>".
...
<div class="corpoTV">
	...
	<div class="slider-for statusDestaque">
		...
		?>
		<div data-codigo="<?=$Codigo[$i]?>" class="listaDestaque" style="opacity:1; margin-bottom:30px; border:solid 1px #333; padding-top:5px; border-top:8px solid <?=$CorDetalhe[$i]?>; border-radius:10px; height:470px;"><?=$TickDetalhe[$i]?></div>
		<?php
		}
		?>
	</div>
...
<div style="position:fixed; bottom:10px; margin-left:40px; margin-right:50px; width:calc(100% - 90px); margin-top:5px;">
